<div class="sub-left-menu scroll">
  <ul class="nav nav-list">
    <li><div class="left-bg"></div></li>
    <li class="time">
      <h1 class="animated fadeInLeft">21:00</h1>
      <p class="animated fadeInRight">Sat,October 1st 2029</p>
    </li>
    <li class="active ripple">
      <a class="nav-header" href="/admin"><span class="fa-home fa"></span> Home 
      </a>
    </li>
    <li class="ripple">
      <a class="tree-toggle nav-header" href="papers">
        <span class="fa-clone fa"></span> Papers 
        <span class="fa-angle-right fa right-arrow text-right"></span>
      </a>
      <ul class="nav nav-list tree">
        <li><a href="papers?type=ne">National exams</a></li>
        <li><a href="papers?type=dte">Driving theory exams</a></li>
        <li><a href="papers?type=tr">Traffic rules</a></li>
      </ul>
    </li>
    <li class="ripple"><a href="subjects"><span class="fa-book fa"></span> Subjects</a></li>
    <li><a href="users"><span class="fa-user fa"></span> Users</a></li>
    <li><a href="subscribers"><span class="fa-envelope fa"></span> Subscribers</a></li>
  </ul>
</div>